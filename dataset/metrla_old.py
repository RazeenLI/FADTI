...
for start_col in range(0, data.shape[2], group_size):
    end_col = min(start_col + group_size, data.shape[2])
    if end_col - start_col < group_size:
        continue  # 最后不足9个传感器的一组跳过

    # 每天的数据 [num_days, 288, 9]
    group_data = data[:, :, start_col:end_col]  # shape: [num_days, 288, 9]
    segments.append(group_data)

# 合并所有组
segments = np.concatenate(segments, axis=0)  # shape: [num_days * num_groups, 288, 9]
print("Final shape:", segments.shape)


# Final shape: (2737, 288, 9)
'''

def get_mean_std():
    df = pd.HDFStore("./data/metr_la/metr-la.h5").get('df')
    data_len = len(df)
...
class MetrLA_Dataset(Dataset):
    def __init__(
        ...
        if mode == 'train':
            ...
            self.observed_mask = ob_mask[-2:]
            self.gt_mask = gt_mask[-2:]
            self.observed_data = c_data[-2:]
...
def get_test_dataloader(batch_size, device, val_len=0.1, test_len=0.2, missing_pattern='block', num_workers=4, missing_ratio=None):
    dataset = MetrLA_Dataset(mode="train", val_len=val_len, test_len=test_len, missing_pattern=missing_pattern,)
    dataset_test = MetrLA_Dataset(mode="test", val_len=val_len, test_len=test_len, missing_pattern=missing_pattern,missing_ratio=missing_ratio)
    test_loader = DataLoader(dataset_test, batch_size=batch_size, num_workers=num_workers, shuffle=False)
