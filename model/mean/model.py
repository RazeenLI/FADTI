...
class Mean(nn.Module):
    ...
    def forward(
        ...
        mean method does not support training, so it only returns 0.
        Args:
    ...
    def compute_feature_means(self, observed_data: torch.Tensor, conditional_mask: torch.Tensor):
        """
        ...
        feature_sum = (data_flat * mask_flat).sum(dim=1)
        means = feature_sum / mask_sum

        return means  # shape: [C]
    ...
    def evaluate(
        ...
        with torch.no_grad():
            ...
            means = self.compute_feature_means(observed_data, cond_mask)
            for i in range(observed_data.size(1)):
                samples[:, i, :][cond_mask[:, i, :] == 0] = means[i]
