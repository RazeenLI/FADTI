...
class denoising_network(nn.Module):
    ...
    def __init__(
        ...
        self.diffusion_embedding = DiffusionEmbedding(
            ...
            embedding_dim=diffusion_embedding_dim, # config["diffusion_embedding_dim"],
        )
            ...
            nn.ModuleList(
                [
                    ResidualBlock(
                        ...
                        diffusion_embedding_dim=diffusion_embedding_dim, # config["diffusion_embedding_dim"],
                        nheads=nheads, # config["nheads"],
                    )
                    for _ in range(layers) # config["layers"])
                ]
            )
        )
